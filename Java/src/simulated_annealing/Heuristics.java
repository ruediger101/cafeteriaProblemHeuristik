    ...
    public static State simulatedAnnealing(State state) {
        ...
        while (tempNow >= finalTemp) { // outer loop , until the current temperature reaches the end temperature
            ...
            for (int k = 0; k < nMarkov; k++) {
                ...
                // --- a new solution is generated by randomly switching customers first element to switch
                ...
                if (nextState.compareTo(currentState) < 0) {
                    ...
                    // tolerance solution ： If the objective function of the new solution is worse than the current solution, the new solution is accepted with a certain
                    // probability
                ...
                if (accept) {
                    ...
                    if (nextState.compareTo(optimalState) < 0) {
                        // if the objective function of the new solution is better than the optimal solution to this point, the new solution is saved as the optimal solution
            ...
            if (logSimulatedAnnealing) {
                ...
                double pBadAccept = ((double) kBadAccept)
                        / ((kBadAccept + kBadRefuse) != 0 ? (kBadAccept + kBadRefuse) : 1); // the acceptance probability of the inferior solution
